...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!user || !pass) {
      return new Response(JSON.stringify({ success: false, error: 'Configuración SMTP no disponible' }), { status: 500 });
    }
    ...
    return new Response(JSON.stringify({ success: false, error: 'Error al enviar el mensaje' }), { status: 500 });
  }
};
